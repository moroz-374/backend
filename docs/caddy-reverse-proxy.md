...
This is the supported automatic-TLS proxy variant for Remnawave Traffic Audit release `2.7.4-traffic-audit.3`. It uses the pinned official `caddy:2.10.2-alpine` image and the existing `remnawave-network` created by the panel Compose project.
...
sudo curl -fsSLo /opt/remnawave/caddy/compose.yml \
  https://raw.githubusercontent.com/moroz-374/backend/2.7.4-traffic-audit.3/deploy/caddy/compose.yml
sudo curl -fsSLo /opt/remnawave/caddy/Caddyfile \
  https://raw.githubusercontent.com/moroz-374/backend/2.7.4-traffic-audit.3/deploy/caddy/Caddyfile
sudo curl -fsSLo /opt/remnawave/caddy/.env \
  https://raw.githubusercontent.com/moroz-374/backend/2.7.4-traffic-audit.3/deploy/caddy/.env.sample

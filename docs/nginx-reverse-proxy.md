...
This is the supported file-based TLS proxy variant for Remnawave Traffic Audit release `2.7.4-traffic-audit.1`. It uses the pinned official `nginx:1.30.3-alpine-slim` image, Certbot on the host and the existing `remnawave-network` created by the panel Compose project.
...
sudo curl -fsSLo /opt/remnawave/nginx/compose.yml \
  https://raw.githubusercontent.com/moroz-374/backend/2.7.4-traffic-audit.1/deploy/nginx/compose.yml
sudo curl -fsSLo /opt/remnawave/nginx/remnawave.conf.template \
  https://raw.githubusercontent.com/moroz-374/backend/2.7.4-traffic-audit.1/deploy/nginx/remnawave.conf.template
sudo curl -fsSLo /opt/remnawave/nginx/.env \
  https://raw.githubusercontent.com/moroz-374/backend/2.7.4-traffic-audit.1/deploy/nginx/.env.sample

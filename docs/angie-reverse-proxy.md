...
This is the supported automatic-TLS Angie variant for Remnawave Traffic Audit release `2.7.4-traffic-audit.2`. It uses the pinned official `docker.angie.software/angie:1.11.8-templated` image, Angie's built-in open-source ACME module and the existing `remnawave-network` created by the panel Compose project.
